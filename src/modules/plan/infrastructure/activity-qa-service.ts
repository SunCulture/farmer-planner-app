...
export async function askActivityQuestion(activityId: string, question: string): Promise<AskQuestionAck> {
  ...
  try {
    const ack = await api.askActivityQuestion(activityId, question)
    ...
    if (err instanceof ActivityQaError) throw err
    throw new ActivityQaError("unknown", err instanceof Error ? err.message : "Ask question failed")
  }
}
...
export async function getQuestionsForActivity(activityId: string): Promise<ActivityQuestion[]> {
  ...
  try {
    ...
    if (err instanceof ActivityQaError) throw err
    throw new ActivityQaError("unknown", err instanceof Error ? err.message : "Fetch questions failed")
  }
}

export async function getDayActivityQuestions(date: string): Promise<DayActivityQuestions[]> {
  try {
    const rows = await api.getDayActivityQuestions(date)
    ...
    return rows
      .map((row) => ({
        ...
        questions: Array.isArray(row?.questions)
          ? row.questions
              .filter((q) => q && typeof q.questionId === "string")
              .map((q) => ({
    ...
    throw new ActivityQaError(
      "unknown",
      err instanceof Error ? err.message : "Fetch day activity questions failed",
    )
  }
}
